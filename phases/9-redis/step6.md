...
- `/docs/ADR.md` — ADR-004의 커맨드 비용 계산 표
...
탭 2개로 같은 회차를 연다:
- A에서 좌석 3개를 홀드 → 3~4초 안에 B에서 회색으로 바뀐다
- B에서 **겹치는 좌석을 포함해** 여러 좌석을 홀드 시도 → 409 → 전체 롤백 + 토스트
- **겹치지 않은 좌석도 부분 홀드되지 않았는지 확인하라.** 이것이 Lua 원자성의 실증이다
...
- Vercel 프로젝트 환경변수에 `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`을 설정한다
- **`NEXT_PUBLIC_` 접두사를 붙이지 마라**
- 재배포 후 배포본에서 항목 1~3을 다시 확인한다
- **재배포 후에도 공연·회차·홀드·예매가 일관되게 남는지** 확인한다 (PRD Day 9의 최종 검증 조건)

Vercel CLI 인증이나 프로젝트 링크가 필요한데 불가능하면 `status: "blocked"`로 표시하고 중단하라. 로컬 검증(항목 1~5)까지의 결과는 summary에 남겨라.

### 7. README 갱신

- 심사자용 Basic Auth 계정 안내가 있는지 확인하고, 없으면 추가한다
- 영속성이 Redis로 전환됐음을 반영한다
- 배포 URL이 최신인지 확인한다
...
수동 검증 결과는 summary에 기록한다. 위 1~7의 각 항목에 대해 통과/실패/미확인을 명시하라.
...
3. 결과에 따라 `phases/9-redis/index.json`의 해당 step을 업데이트한다:
   ...
   - 사용자 개입 필요 (Upstash 자격증명, Vercel 인증 등) → `"status": "blocked"`, `"blocked_reason": "구체적 사유"` 후 즉시 중단
...
- 검증 중 발견한 버그를 이 step에서 대규모로 고치지 마라. 이유: 이 step은 검증과 배포다. 버그를 발견하면 summary에 기록하고, 사소한 수정이 아니면 별도 작업으로 남겨라
